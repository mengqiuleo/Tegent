...
import type { ConsentPreview, PluginScope, PluginSource } from '@tegent/core'
...
async function cliInstall(args: string[]): Promise<number> {
  ...
  try {
    const result = await installPlugin({
      ...
      // values by editing ~/.tegent/plugins/user-config.json directly
...
async function promptUserConfig(
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    process.stderr.write(
      chalk.yellow(
        'No TTY — skipping userConfig prompt. Pre-seed values in ~/.tegent/plugins/user-config.json or use --yes.\n',
      ),
    )
    return {}
  }
...
async function cliDoctor(): Promise<number> {
  ...
  console.log()
  console.log('For deeper diagnostics, set DEBUG_STDOUT=1 and check ~/.tegent/logs/debug.log')
