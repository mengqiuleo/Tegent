...
//   2. `~/.tegent/config.json` `model` field — written by `/model` picker
 ...
 *   2. `~/.tegent/config.json` `model` field (written by the /model picker)
 *   3. `X_CODE_MODEL` environment variable
...
// ── User config file (~/.tegent/config.json) ────────────────────────────
//
